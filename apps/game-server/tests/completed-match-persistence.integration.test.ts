...
function toComparableMap(rows: LeaderboardSnapshotRow[]) {
  return new Map(rows.map((row) => [row.userId, row]));
}

async function readLeaderboardSnapshot(userIds: string[]) {
...
async function restoreLeaderboardSnapshot(rows: LeaderboardSnapshotRow[]) {
  const prisma = getGameServerPrismaClient();
...
function diffLeaderboardSnapshots(before: LeaderboardSnapshotRow[], after: LeaderboardSnapshotRow[]) {
  ...
  return PERSISTENCE_TEST_PLAYERS.map((player) => {
    ...
    return {
      ...
      lastMatchAtChanged:
        String(beforeRow.lastMatchAt?.getTime() ?? null) !==
        String(afterRow.lastMatchAt?.getTime() ?? null)
    };
  });
}

function shouldLastMatchAtAdvance(lastMatchAt: Date | null, finishedAt: number): boolean {
  return lastMatchAt === null || lastMatchAt.getTime() < finishedAt;
}

function buildExpectedFirstPersistDeltas(
  userIds: { an: string; binh: string; chi: string; dung: string },
  beforeStats: LeaderboardSnapshotRow[],
  finishedAt: number
) {
...
test(
  "finished room persistence writes matches, match_players, and leaderboard updates",
...
test(
  "persisting the same finished room twice is a no-op for rows and leaderboard counters",
  persistenceTestOptions,
  async () => {
    const cleanup = new TestCleanupStack();
    const prisma = getGameServerPrismaClient();
