...
import {
  ...
  type GameErrorEvent,
  type GamePlayerConnectionChangedEvent
...
import {
  createSeededMatchJoinOptions,
  createTestClient,
  FakeRoomClock,
  getBroadcastPayloads,
  getSentMessages,
  withMockedMathRandom
} from "./support/index.js";

function getMessageHandler(room: ReturnType<typeof createSeededMonopolyRoom>["room"], command: string) {
  const handler = (room.messages as Record<string, ((...args: unknown[]) => void) | undefined>)[command];
  assert.ok(handler, `Expected message handler for ${command}.`);
  return handler;
}

function setRoomClock(room: ReturnType<typeof createSeededMonopolyRoom>["room"], clock: FakeRoomClock): void {
  (room as unknown as { clock: FakeRoomClock }).clock = clock;
}
...
test("happy path gameplay commands mutate room state and emit expected events", async () => {
  ...
  room.onJoin(client as never, createSeededMatchJoinOptions({ playerId: "p1", matchId: room.state.matchId }));
  ...
  assert.deepEqual(
    broadcastRecorder.broadcasts.map((record) => record.eventName),
    [GAME_DICE_ROLLED_EVENT, GAME_PLAYER_MOVED_EVENT, GAME_TILE_RESOLVED_EVENT]
  );
  ...
  assert.deepEqual(
    broadcastRecorder.broadcasts.map((record) => record.eventName),
    [GAME_PAYMENT_APPLIED_EVENT, GAME_PROPERTY_PURCHASED_EVENT]
  );
...
test("inactive player commands are rejected without mutating room state", () => {
  ...
  room.onJoin(activeClient as never, createSeededMatchJoinOptions({ playerId: "p1", matchId: room.state.matchId }));
  room.onJoin(inactiveClient as never, createSeededMatchJoinOptions({ playerId: "p2", matchId: room.state.matchId }));
  ...
  const errors = getSentMessages<GameErrorEvent>(inactiveClient, GAME_ERROR_EVENT);
...
test("unknown players are rejected from joining an active seeded room", () => {
  ...
  assert.throws(
    () => {
      room.onJoin(
        stranger as never,
        createSeededMatchJoinOptions({ playerId: "stranger", matchId: room.state.matchId })
      );
    },
    (error: unknown) => {
      assert.equal((error as { code?: number }).code, ErrorCode.MATCHMAKE_EXPIRED);
      assert.match(String((error as { message?: string }).message), /not part of this active match room/i);
      return true;
    }
  );
});
...
test("reserved seats can be reclaimed without creating duplicate player entries", () => {
  ...
  const connectionChanges = getBroadcastPayloads<GamePlayerConnectionChangedEvent>(
    broadcastRecorder,
    GAME_PLAYER_CONNECTION_CHANGED_EVENT
  );
...
test("disconnect expiry resolves through authoritative abandonment and emits finish events", async () => {
  ...
  await room.onDrop(client as never, 1001);
