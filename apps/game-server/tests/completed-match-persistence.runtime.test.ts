...
import type { CompletedMatchPersistenceSnapshot } from "../src/persistence/contracts.js";
import type {
  CompletedMatchPersistenceRepository,
  PersistedCompletedMatchRecord
} from "../src/persistence/repositories/completed-match.repository.js";
...
async function withDatabaseUrl<T>(databaseUrl: NullableString, callback: () => Promise<T>): Promise<T> {
...
interface MockRepositoryOptions {
  findResult?: PersistedCompletedMatchRecord | null;
  onPersist?: (snapshot: CompletedMatchPersistenceSnapshot) => Promise<void> | void;
}
...
function createMockRepository(options: MockRepositoryOptions = {}) {
  const calls = {
    find: 0,
    persist: 0,
    snapshots: [] as CompletedMatchPersistenceSnapshot[]
  };

  const repository: CompletedMatchPersistenceRepository = {
...
test("repository failure marks room failed and keeps the room retryable", async () => {
  await withDatabaseUrl("postgresql://mock-user:mock-pass@localhost:5432/mock-db", async () => {
    ...
    await assert.rejects(
      () => persistCompletedMonopolyRoomIfNeeded(room, repository),
...
test("failed persistence can be retried successfully on the same finished room", async () => {
  await withDatabaseUrl("postgresql://mock-user:mock-pass@localhost:5432/mock-db", async () => {
    ...
    await assert.rejects(
      () => persistCompletedMonopolyRoomIfNeeded(room, failing.repository),
