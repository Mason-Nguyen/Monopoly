import { LOBBY_ERROR_EVENT, LOBBY_MATCH_START_FAILED_EVENT, LOBBY_MATCH_STARTING_EVENT, LOBBY_SET_READY_COMMAND, LOBBY_START_MATCH_COMMAND } from "@monopoly/shared-types";
...
export function syncLobbyState(room) {
    updateLobbyComputedState(room);
    room.refreshMetadata();
}
export function createLobbyMessageHandlers(room) {
    return {
        [LOBBY_SET_READY_COMMAND]: (client, payload) => {
        ...
        [LOBBY_START_MATCH_COMMAND]: async (client) => {
            const playerId = getPlayerIdFromClient(client);
            ...
            try {
                ...
                room.refreshMetadata();
                room.broadcast(LOBBY_MATCH_STARTING_EVENT, {
                ...
                room.broadcast(LOBBY_MATCH_START_FAILED_EVENT, {
