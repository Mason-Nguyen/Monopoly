import { GAME_DICE_ROLLED_EVENT, GAME_PAYMENT_APPLIED_EVENT, GAME_PLAYER_ELIMINATED_EVENT, GAME_PLAYER_MOVED_EVENT, GAME_PROPERTY_PURCHASED_EVENT, GAME_RESULT_READY_EVENT, GAME_TILE_RESOLVED_EVENT } from "@monopoly/shared-types";
function getPlayerLabel(roomState, playerId) {
    return roomState.players.get(playerId)?.displayName || playerId;
}
function getTileName(roomState, tileIndex) {
    return roomState.board.tiles.find((tile) => tile.tileIndex === tileIndex)?.name || `Tile ${tileIndex}`;
}
function resolveTileSummaryCode(event) {
...
function createTileResolvedMessage(roomState, event, summaryCode) {
...
function mapEngineEventToBroadcastInstructions(roomState, event) {
    switch (event.type) {
        case "dice_rolled":
            return [
                {
...
export function syncMonopolyRoomMetadata(room) {
    room.metadata = {
        ...(room.metadata ?? {}),
        roomKind: "monopoly",
...
export function broadcastEngineTransitionEvents(room, transitionResult) {
