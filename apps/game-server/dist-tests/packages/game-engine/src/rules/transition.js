...
import { canEndTurn, createAwaitEndTurnState } from "../reducers/turn.js";
import { advanceToNextPlayableTurn, getTileConfigOrThrow, purchasePropertyOrThrow, resolveActivePlayerTile } from "../resolvers/index.js";
import { getAvailableActions } from "./available-actions.js";
import { EngineRuleError } from "./errors.js";
function assertBoardMatchesState(state, boardConfig) {
...
function assertActiveAction(state, action) {
...
function applyRollDiceTransition(input) {
    ...
    const tileResolution = resolveActivePlayerTile(nextState, boardConfig, activePlayer.playerId);
...
function applyBuyPropertyTransition(input) {
    ...
    const nextState = cloneMatchState(state);
    const purchaseResult = purchasePropertyOrThrow(nextState, boardConfig, action.actingPlayerId, action.propertyId);
...
function applySkipOptionalActionTransition(input) {
...
function applyEndTurnTransition(input) {
...
export function applyEngineAction(input) {
