import type { BoardConfig, TileConfig } from "@monopoly/shared-types";

function countTilesByType(tiles: TileConfig[], tileType: TileConfig["tileType"]): number {
  return tiles.filter((tile) => tile.tileType === tileType).length;
}

export function validateBoardConfig(boardConfig: BoardConfig): string[] {
  const errors: string[] = [];
...
export function assertValidBoardConfig(boardConfig: BoardConfig): BoardConfig {
