...
import type { BoardConfig, PlayerId, PropertyId, TileConfig } from "@monopoly/shared-types";
import type {
  EngineEvent,
...
import {
  ...
  createAwaitOptionalActionTurnState
...
import {
  resolveMandatoryBankPayment,
  resolveMandatoryPlayerPayment
} from "./economy.js";

export interface EngineTileResolutionResult {
  events: EngineEvent[];
  activePlayerEliminated: boolean;
}

function getActivePlayerOrThrow(
...
function createTileResolvedEvent(
  playerId: PlayerId,
  tile: TileConfig,
  options?: {
...
export function resolveActivePlayerTile(
  state: EngineMatchState,
  boardConfig: BoardConfig,
  playerId: PlayerId
): EngineTileResolutionResult {
  ...
  switch (tile.tileType) {
    ...
    case "tax": {
      ...
      const paymentResolution = resolveMandatoryBankPayment(
        state,
        playerId,
        tile.taxAmount,
        "tax"
    ...
    case "property": {
      ...
      if (!propertyOwnership) {
        throw new EngineRuleError(
          `Property ownership state is missing for ${tile.propertyId}.`
        );
      }
      ...
      const propertyConfig = getPropertyConfigOrThrow(boardConfig, tile.propertyId);
      const rentResolution = resolveMandatoryPlayerPayment(
        state,
        playerId,
        propertyOwnership.ownerPlayerId,
        propertyConfig.rentAmount,
        "rent"
      );
    ...
    case "go_to_jail": {
      ...
      const resolvedEvent = createTileResolvedEvent(playerId, tile);
      const forcedMovementEvent: EnginePlayerMovedEvent = {
        type: "player_moved",
        playerId,
      ...
      const jailStateChangedEvent: EngineJailStateChangedEvent = {
