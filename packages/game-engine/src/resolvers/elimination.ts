import type { MatchEndReason, PlayerId, UnixTimestampMs } from "@monopoly/shared-types";
...
function getPlayerOrThrow(state: EngineMatchState, playerId: PlayerId) {
  ...
  for (const propertyId of releasedPropertyIds) {
    ...
    if (!ownership) {
      throw new EngineRuleError(
        `Property ownership state is missing for released property ${propertyId}.`
      );
    }
...
export function finishMatchIfSinglePlayerRemaining(
  state: EngineMatchState,
  now: UnixTimestampMs
): EngineMatchEndedEvent | null {
